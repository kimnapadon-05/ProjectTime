...
function loadSheet() {
    ...
    $.ajax({
        ...
        url: window.PROJECT_ROOT + '/backend/water_recording_handler.php',
        method: 'POST',
        ...
        success: function(data) {
            ...
            if (data.length === 0) {
                ...
                data.forEach((item) => {
                    let bgClass = item.is_saved ? 'table-primary' : ''; // เปลี่ยนสีไฮไลท์เป็นสีฟ้าอ่อน
                    ...
                    rows += `
                        <tr class="${bgClass}" id="row_${item.history_id}">
                            ...
                            <td><span class="badge bg-info text-dark">${item.meter_serial}</span></td>
...
// ฟังก์ชันคำนวณ (Logic เดียวกันกับไฟฟ้า)
function calculateUsage(id) {
    ...
    if (!isNaN(curr)) {
        let usage = curr - prev;
        if (usage < 0) {
            $(`#usage_${id}`).html('<span class="text-danger">ติดลบ</span>');
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.status === 'success') {
                $(`#row_${id}`).addClass('table-primary');
                ...
                Toast.fire({ icon: 'success', title: 'บันทึกค่าน้ำสำเร็จ' });
            } else {
