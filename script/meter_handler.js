...
function loadMeterTable(page, search = '') {
    ...
    $.ajax({
        url: window.PROJECT_ROOT + '/backend/meter_handler.php', 
        method: 'POST', 
        data: { 
            action: 'read',
            page: page,
...
function loadOptions() {
    $.ajax({
        url: window.PROJECT_ROOT + '/backend/meter_handler.php', method: 'POST', data: { action: 'get_options' }, dataType: 'json',
        success: function(res) {
            if(res.error) {
...
function saveMeter() {
    ...
    $.ajax({
        url: window.PROJECT_ROOT + '/backend/meter_handler.php', method: 'POST', data: $('#meterForm').serialize(), dataType: 'json',
        success: function(res) {
...
function deleteMeter(id) {
    Swal.fire({ title: 'ยืนยันการลบ?', icon: 'warning', showCancelButton: true, confirmButtonText: 'ลบ', cancelButtonText: 'ยกเลิก' }).then((r) => {
        if(r.isConfirmed) {
            $.post(window.PROJECT_ROOT + '/backend/meter_handler.php', {action:'delete', meter_id:id}, (res) => {
                if (res.status === 'success') {
